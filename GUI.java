...
public class GUI extends JFrame implements KeyListener {
	...
	public char keyChar;
	// Counters: i = location of which character you are on for the word at
	// hand, ii = check against length of word, iii = iteration of
	// checkNoNewNumbers method.
	private int i = 0;
	private int ii = i + 1;
	private int iii = 0;
	...
	public void resetAll() {
		flagNoNewNumbers.clear();
		resetWord();
		i=0;
		iii=0;
		}

	// part of the interface for KeyListener, this is where the logic is
	// executed
	...
	void checkFinish() {
		if (flagNoNewNumbers.contains(wordPulled) && (flagNoNewNumbers.size() == wordListObj.arrLength) && (iii == 0)) {
			...
			resetAll();
			AppStart.start = System.currentTimeMillis();
			}
